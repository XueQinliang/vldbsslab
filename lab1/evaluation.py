...
if __name__ == '__main__':
    ...
    for item in test_data:
        range_query = rq.ParsedRangeQuery.parse_range_query(item['query'])
        est_avi.append(
            stats.AVIEstimator.estimate(range_query, table_stats) *
            table_stats.row_count)
        est_ebo.append(
            stats.ExpBackoffEstimator.estimate(range_query, table_stats) *
            table_stats.row_count)
        est_min_sel.append(
            stats.MinSelEstimator.estimate(range_query, table_stats) *
            table_stats.row_count)
        act.append(item['act_rows'])

    est_spn_sample_1000 = est_spn('./data/title_sample_1000.csv', test_data,
                                  table_stats)
    est_spn_sample_10000 = est_spn('./data/title_sample_10000.csv', test_data,
                                   table_stats)
    est_spn_sample_20000 = est_spn('./data/title_sample_20000.csv', test_data,
                                   table_stats)

    # _, _, est_mlp, _ = learn_from_query.est_mlp(train_data, test_data, table_stats, columns)
    _, _, est_xgb, _ = learn_from_query.est_xgb(train_data, test_data,
                                                table_stats, columns)
